...
@tags_router.post(
    "/{tag_uid}/books/{book_uid}", response_model=Book, dependencies=[user_role_checker]
)
async def add_tag_to_book(
    tag_uid: str, book_uid: str, session: AsyncSession = Depends(get_session)
) -> Book:
    tag_data = TagAddModel(tag_uids=[tag_uid])
